...
namespace hamming_code
{
    ...
    class HammingCoding : encryption.SpecialMath
    {
        ...
        public static string Coding(string PATH, string message, List<Node> Alphabet)
        {
            ...
            {
                if ((help = FindCodeOfSymbol(Alphabet, message[i].ToString())) != System.String.Empty)
                    output_buf += help + ":";
            }
    ...
    class HammingDecoding
    {
        ...
        public static string DecodingProcedure(string PATH, string message, List<Node> Alphabet, Matrix H, ref string errors)
        {
            // Вспомогательные переменные
            ...
            for (int i = 0; i < message.Length; i++)
            {
                ...
                else
                {
                    ...
                    help2 = FindingAndfixingErrors(help1, H, count, ref errors);
        ...
        public static int[] FindingAndfixingErrors(string code, Matrix H, int count, ref string errors)
        {
            int[] arr = new int[H.M];
            ...
            errors += "Исправлена ошибка в " + count + " слове (" + index + " бит);  ";
